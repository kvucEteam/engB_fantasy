		...
		events : {
			...
			'onReady' : function(event) {
				...
				setInterval(function() {
					...
					$('#time').html(m + ":" + dec_s + " / " + vid_length + " [Next question in <b> " + (timestamp_Array[runde] - playTime) + "</b> seconds]");
...
function init(tal, taeller) {
	...
	$("tr").click(function() {
		...
		if(valgt == bol) {
			...
			$("#feed").html("CORRECT!").css("color", "green").css("font-size","12px");
			spm_taeller++;
			$("tr").each(function() {
				if($(this).attr("id") == bol) {
			...
			$("#feed").html("That's not correct. Please try again.").css("color", "red").css("font-size","12px");
			score++;
...
function slutFeedback() {
	...
	$(".popud").html("<h3 class = 'forfra'>You have completed the quiz. <br>You made " + score + " errors in the quiz.<br/>Click to try again</h3>");
